...
int main(int argc, char* argv[]) {
    //check for arguments
    if (argc != 3) {
        cerr << "Usage: " << argv[0] << " <users filename> <posts filename>" << endl;
        return 1;
    }
    ...
    while (true) {
        ...
        if (option == 1) {
            ...
            string lastName;
            int number;
            if(ss >> firstName >> lastName >> number){
                int id = net.getId(firstName + " " + lastName);
                if(id >= 0){
                    cout << endl;
                    cout << net.getPostsString(id, number) << endl;;
                }
                else{
                    cout << "Error: Cannt find user" << endl;
                }
            }
        }else {
            break;
